...
def semi_ici(X, Y, q, q_ys, labelled_samples):
    ici = ICI(classifier='lr', num_class=n_ways, step=3, reduce='pca', d=5)
    ...
    for i in range(X.shape[0]):
        ...
        ici.fit(support_features, support_ys)
        unlabelled_ys_pred = ici.predict(unlabelled_features, None, False, unlabelled_ys)
...
# ---- data loading
    ...
    query_data = centerDatas(query_data)
...
# switch to cuda
    ...
    #acc_mine, acc_std = semi_pt(ndatas, olabels, query_data, q_labels)
    #acc_ici, acc_std_ici = semi_ici(ndatas, labels, query_data, q_labels, n_lsamples)
